...
public final class SampleBufferFloatTest extends SampleBufferContract
{
  @Test
  public void testChannelCountWrong()
  {
    final var ex =
      Assertions.assertThrows(
        ...
        () -> SampleBufferFloat.createWithByteBuffer(
          -1,
          100L,
          44100.0,
  ...
  public void testChannelCountWrongDirect()
  {
    final var ex =
      Assertions.assertThrows(
        RangeCheckException.class,
        () -> SampleBufferFloat.createWithDirectBuffer(
          -1,
          100L, 44100.0));
  ...
  protected SampleBufferType createBuffer(
    ...
    final long frames)
  {
    return SampleBufferFloat.createWithHeapBuffer(channels, frames, 44100.0);
  }

  @Test
  public void testCreateHeapMonoSimple()
  {
    final var buffer = SampleBufferFloat.createWithHeapBuffer(1, 100L, 44100.0);
  ...
  {
    final var buffer = SampleBufferFloat.createWithHeapBuffer(2, 100L, 44100.0);
  ...
  {
    final var buffer = SampleBufferFloat.createWithDirectBuffer(1, 100L, 44100.0);
    Assertions.assertEquals(1, buffer.channels());
    Assertions.assertEquals(100L, buffer.frames());
    Assertions.assertEquals(100L, buffer.samples());
  }
  ...
  {
    final var buffer = SampleBufferFloat.createWithDirectBuffer(2, 100L, 44100.0);
    Assertions.assertEquals(2, buffer.channels());
    Assertions.assertEquals(100L, buffer.frames());
    Assertions.assertEquals(200L, buffer.samples());
  }
